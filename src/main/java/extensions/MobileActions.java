...
public class MobileActions extends CommonOps {
    ...
    @Step("MultiTap on Element")
    public static void multiTap(MobileElement elem){
        wait.until(ExpectedConditions.elementToBeClickable(elem));
        TouchAction action = new TouchAction(mobileDriver);
        action.tap(new TapOptions().withElement(ElementOption.element(elem)).withTapsCount(2)).perform();
    }

    @Step("Press on Element")
    public static void Press(MobileElement elem){
        wait.until(ExpectedConditions.elementToBeClickable(elem));
        TouchAction action = new TouchAction(mobileDriver);
        action.press(new ElementOption().withElement(elem)).waitAction(WaitOptions.waitOptions(Duration.ofSeconds(2))).perform();
    }
    ...
    public static void longPress(MobileElement elem){
        wait.until(ExpectedConditions.elementToBeClickable(elem));
        TouchAction action = new TouchAction(mobileDriver);
        action.longPress(new ElementOption().withElement(elem)).waitAction(WaitOptions.waitOptions(Duration.ofSeconds(2))).perform();
    }

    @Step("Drag&Drop on Element")
    public static void dragAndDrop(MobileElement elem1,MobileElement elem2){
        wait.until(ExpectedConditions.elementToBeClickable(elem1));
        TouchAction action = new TouchAction(mobileDriver);
        action.press(new ElementOption().withElement(elem1)).moveTo(new ElementOption().withElement(elem2)).release().perform();
    }
